...
trigger: 用户要求处理欧盟MDR事务，提到MDR、CE认证、技术文件、临床证据、UDI、EUDAMED、公告机构审核、上市后监督等
do-not-trigger: 用户要求处理FDA法规、ISO认证、非医疗器械相关的法规事务，或与MDR合规无关的编码任务
user-invocable: true
---
...
- `udi-eudamed-compliance.py`: UDI and EUDAMED compliance verification
...
- `technical-documentation-guide.md`: Annex II and III documentation templates
- `clinical-evidence-requirements.md`: Clinical evaluation and PMCF guidance
- `gspr-guide.md`: General Safety and Performance Requirements guide
- `post-market-surveillance-guide.md`: PMS and vigilance requirements
- `eudamed-udi-guide.md`: UDI system and EUDAMED registration
- `mdcg-guidance-summary.md`: MDCG guidance compilation

### assets/templates/
- `mdr-classification-decision.md`: Classification decision template
- `technical-file-checklist.md`: Technical file preparation checklist
- `clinical-evaluation-report.md`: Clinical evaluation report template
- `pmcf-plan-template.md`: PMCF plan template
- `psur-template.md`: Periodic safety update report template
- `gspr-checklist.md`: GSPR compliance checklist